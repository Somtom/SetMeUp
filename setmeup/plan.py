...
class PlanStepChecksum:
    ...
    def test_if_checksums_equal(self, checksum_value):
        if self.checksum_type == PlanStepChecksum.TYPE_FILE:
            return checksum_from_file(self.origin) == checksum_value
        elif self.checksum_type == PlanStepChecksum.TYPE_STRING:
...
class PlanStep:
    ...
    def __init__(
        ...
        self.name = name
        self.description = description
        self.checksum = PlanStepChecksum(**checksum)
...
class Plan:
    ...
    _filename: str = DEFAULT_PLAN_FILE_NAME

    def __init__(
            self,
            setmeup_version: Optional[str] = None,
            required_env_vars: Optional[List[PlanEnvironmentVariable]] = None,
            steps_to_execute: Optional[List[PlanStep]] = None,
        ...
        self._filename = filename or DEFAULT_PLAN_FILE_NAME
        self.logger = logging.getLogger(__name__)
    ...
    def apply(self) -> None:
        ...
        for step in self.steps_to_execute:
            ...
            checksum_data: PlanStepChecksum = step.checksum
